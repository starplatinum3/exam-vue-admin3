...
export default {
  ...
  mounted () {
    ...
    dashboardApi.index().then(re => {
      ...
      _this.echartsUserAction.setOption(this.option('用户活跃度-这个月的每一天', '{b}日{c}度', response.mothDayText, response.mothDayUserActionValue))
      _this.echartsQuestion.setOption(this.option('题目月数量-这个月的每一天', '{b}日{c}题', response.mothDayText, response.mothDayDoExamQuestionValue))
      this.loading = false
