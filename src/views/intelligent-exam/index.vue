            ...
            <el-form :model="form" ref="form" label-width="25%">
              ...
              <el-form-item label="选择题：" prop="choiceNum" required>
                  <div class="slider-block">
                    <el-slider v-model="form.choiceNum" :step="3" show-stops :max="15"></el-slider>
                  </div>
              </el-form-item>
              <el-form-item label="填空题：" prop="fillingNum" required>
                  <div class="slider-block">
                    <el-slider v-model="form.fillingNum" :step="1" show-stops :max="10"></el-slider>
                  </div>
              </el-form-item>
              <el-form-item label="简答题：" prop="answerNum" required>
                  <div class="slider-block">
                    <el-slider v-model="form.answerNum" :step="1" show-stops :max="10"></el-slider>
                  </div>
              </el-form-item>
              <el-form-item label="难度：" prop="difficulty" required>
                <el-radio-group v-model="form.difficulty">
                  <el-radio-button v-for="item in difficultEnum" :key="item.key" :label="item.key">{{item.value}}</el-radio-button>
                </el-radio-group>
              </el-form-item>
              <el-form-item>
...
export default {
  ...
  methods: {
    ...
    submitForm () {
      ...
      this.$refs.form.validate((vaild) => {
        if (vaild) {
          this.listLoading = true
          examPaperApi.examInsert(this.form).then(data => {
            if (data.code === 1) {
    ...
    subjectChange () {
      ...
      this.chapterFilter = this.chapters.filter(data => data.subjectId === this.form.subjectId * 1)
  ...
  computed: {
    ...
    ...mapState('enumItem', { difficultEnum: state => state.exam.examPaper.difficultEnum })
  }
}
</script>
