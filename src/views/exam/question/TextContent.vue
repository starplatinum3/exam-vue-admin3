      ...
      <el-table-column label="操作" align="center" width="220px">
        <template slot-scope="{ row }">
          ...
          <el-button size="mini" @click="editQuestion(row)">编辑</el-button>
           <el-button size="mini" @click="lookQuestuon(row)">lookQuestuon</el-button>
           <el-button size="mini" @click="lookPaper(row)">lookPaper</el-button>
          <el-button
            size="mini"
            type="danger"
...
this.queryParam.pageIndex=res.response.current
    ...
    contentParsedListParse(contentParsedList){
      for(let contentParsedItem of contentParsedList){
        console.log(contentParsedItem);
        if(contentParsedItem.name){
          return "考试题目"
        }
        // contentParsedItem.name = contentParsedItem.name.replace(/<[^>]+>/g,"");
        // o.questionItemObjects = JSON.parse(o.content).questionItemObjects
        // o.titleContent = JSON.parse(o.content).titleContent
        // o.analyze = JSON.parse(o.content).analyze
      }
    },
    typeMake(tableData){

      for(let textContent of tableData){
       let contentParsed=    JSON.parse( textContent.content)
      //  this.parseContent
      console.log("contentParsed");
      console.log(contentParsed);
      let  contentTypeName= this.contentParsedListParse(contentParsed)
      textContent.contentTypeName=contentTypeName
      let dd=[
    {
        "name": "面经 前端",
        "questionItems": [
    ...
    lookQuestuon(row) {
// /exam/paper/edit?id=96
    
      let infoTextContentId= row.id
      let  dataPost= {  
         "correct":null ,
         "createTime":null ,
         "createUser":null ,
      ...
      questionApi.selectPageEqual({ pageNumber: 0, pageSize: 10 }, dataPost)
      .then((res) => {
        ...
        // let  objQuestion= res.response
        // this.tableData =  res.response.records
        let   objQuestion =  res.response.records[0]
// this.total =res.response.total
// this.queryParam.pageIndex=res.response.current
this.routerPushQuestion(objQuestion)
      })
      // row.id
    },
    editQuestion(row) {
